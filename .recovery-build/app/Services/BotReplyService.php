...
class BotReplyService
{
    ...
    public function generate(Bot $bot, string $userContent, ?Conversation $conversation = null): BotReply
    {
        ...
        $matchedProducts = $this->findProducts($bot, $searchQuery, $context, $conversation);

        if (! empty($matchedProducts) && $this->products->isCardRequest($userContent)) {
            return new BotReply(
        ...
        try {
            ...
            if ($reply !== null) {
                ...
                return new BotReply($reply, $matchedProducts);
            }
        } catch (\Throwable $e) {
    ...
    private function findProducts(Bot $bot, string $searchQuery, string $context, ?Conversation $conversation): array
    {
        ...
        if (empty($matchedProducts) && $context !== '') {
            $matchedProducts = $this->products->extractFromText($context, $searchQuery);
        }

        if (empty($matchedProducts) && $conversation) {
